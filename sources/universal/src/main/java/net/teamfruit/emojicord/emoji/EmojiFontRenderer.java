...
public class EmojiFontRenderer extends FontRenderer {
	...
	private final Map<Integer, EmojiObject> emojis = new HashMap<>();
	...
	private String getEmojiFormattedString(String text) {
		if (EmojicordConfig.renderEnabled&&!StringUtil.isNullOrEmpty(text)) {
			if (StringUtil.isNullOrEmpty(text))
				return text;
			final Pair<String, List<Pair<EmojiId, String>>> emojiPair = TextParser.parse(text);
			text = emojiPair.getLeft();
			final Matcher matcher = TextParser.placeHolderPattern.matcher(text);
			...
			for (final Pair<EmojiId, String> entry : emojis)
				if (matcher.find()) {
					...
					final EmojiObject emoji = emojiId==null ? null : EmojiCache.instance.getEmoji(emojiId);
					if (emoji==null)
	...
	private float renderChar(final char c, final boolean italic, final int index, final boolean hasShadow) {
		if (EmojicordConfig.renderEnabled) {
			final EmojiObject emoji = this.emojis.get(Integer.valueOf(index));
			if (emoji!=null)
				if (hasShadow)
					return 10.0F;
				else {
	...
	private void renderEmoji(final EmojiObject emoji) {
